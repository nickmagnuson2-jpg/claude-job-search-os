...
| `act_apply.py` | pipeline-add / contact-add / notes-add / company-note-add for inbox routing (`--repo-root`/`--dry-run` go BEFORE the subcommand) |
...
|---|---|---|
| `/plan-hardening` | Adversarial panel over N rounds until a fresh judge finds no blocking holes | Before executing an expensive or irreversible plan |
...
| `tools/.pii-denylist.txt` | Generated PII tokens | Regenerate with `gen_pii_denylist.py` |
