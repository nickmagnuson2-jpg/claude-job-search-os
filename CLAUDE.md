...
3. **Add a one-line index pointer** (`- [Title](feedback_*.md) — terse hook`) to the matching `memory/index-<topic>.md` shard — **NOT** to MEMORY.md, which since the 2026-07-08 restructure holds ONLY Critical Context (facts that must never depend on recall — employment status, family contacts, active hard-rule-DUE items) plus the Topic Shards router, and is no longer the index. Shards: outreach / coaching / research / tools / hooks / repo-ops / agents / verification / system / personal / projects (11 since the 2026-08-13 split). Add directly to MEMORY.md's Critical Context block only if the fact itself belongs there. Keep each shard under ~24KB; archive on overflow per [[project_memory_directory_structure]].
...
memory/            ⚠️ THREE PHYSICAL ROOTS, not one. Resolve before asserting any file is absent.
  ...
  └─ <repo>/memory/canonical-sidecar/ + global-claude-mirror/
                       ...
                       they are load-bearing for backup coverage.
