...
- **Generating a tailored CV:** Run `/generate-cv <job-url-or-jd>` to produce a role-specific CV and interview cheat sheet. Follows the 11-step resume workflow, saves to `output/`, and updates the pipeline CV Used field.
- **Preparing for an interview:** Run `/prep-interview <company>` when an interview is scheduled. Uses 3 parallel agents to map questions to coached answers, condense company context, and build a pre-call tactics doc. Saves to `output/`.
- **Weekly retrospective:** Run `/weekly-review` to surface stalled pipeline entries, outreach gaps, task velocity, and a ranked priority list for the coming week. Logs to `data/weekly-review-log.md`.
