...
│   ├── application-workflow.md   # Shared standards for /generate-cv, /apply, /cover-letter (tailoring rules, quality checks, cheat sheet)
...
├── .claude/
│   └── skills/                  # 27 skill definitions powering slash commands — see Working With This Repo
├── memory/
...
│   ├── pipe_write.py          # Atomic add/update/remove for data/job-pipeline.md → JSON (used by /pipe)
│   ├── networking_write.py    # Atomic add/log/remove for data/networking.md → JSON (used by /networking)
│   ├── remember_apply.py      # 8 destination handlers for note routing → JSON (used by /remember)
│   ├── act_apply.py           # pipeline-add/contact-add/notes-add for inbox routing → JSON (used by /act)
│   ├── gmail_fetch.py         # Incremental Gmail OAuth sync → inbox/ files (run by n8n or Task Scheduler)
│   ├── run_gmail_fetch.bat    # Legacy Task Scheduler wrapper (disabled — n8n handles this now)
│   ├── run_n8n.bat            # n8n startup script — sets NODES_EXCLUDE=[] to unlock Execute Command node
│   ├── n8n_outreach_nudge.py  # n8n wrapper: writes inbox nudge if overdue follow-ups found
│   ├── n8n_dossier_nudge.py   # n8n wrapper: writes inbox nudge if stale dossiers found
│   ├── n8n_weekly_reminder.py # n8n wrapper: writes Friday weekly-review reminder to inbox/
│   └── .cache/                # Pre-computed JSON from n8n Standup Cache Warm workflow
...
Shared application standards — candidate context loading, tailoring rules, CV quality standards (16-point checklist), and cheat sheet structure — live in `framework/application-workflow.md`. The three application skills (`/generate-cv`, `/apply`, `/cover-letter`) reference this framework instead of duplicating rules. Each skill's workflow steps remain inline in its own SKILL.md.
...
- **Manage tasks:** `/todo` — view active to-dos with pipeline and contact cross-references. `/todo add`, `/todo done`.

### Applications

- **One-command apply bundle:** `/apply <job-url-or-jd> [context]` — generates CV + cover letter, adds to pipeline, all in one step.
...
- **Morning briefing:** `/standup` — pipeline health, top 3 actions, pending outreach, momentum read. Run at session start.
- **End-of-day close-out:** `/checkout` — writes daily log, surfaces tomorrow's top 3 priorities, optional reflection. Bookend to `/standup`.
...
**Atomic write scripts:** Four scripts handle all data file mutations for the search operations layer. Each returns JSON and is gated — skills check `status: ok` before proceeding:
- **`tools/pipe_write.py`** — `add "<company>"`, `update "<company>" <stage>`, `remove "<company>"` → `data/job-pipeline.md`
- **`tools/networking_write.py`** — `add "<name>"`, `log "<name>" <entry>`, `remove "<name>"` → `data/networking.md`
- **`tools/remember_apply.py`** — routes classified notes to 8 possible destinations (pipeline, networking, profile, notes, outreach-log, etc.)
- **`tools/act_apply.py`** — `pipeline-add`, `contact-add`, `notes-add` for inbox item routing; used by `/act` Step 4

All follow the same pattern: `PYTHONIOENCODING=utf-8 python tools/<script>.py <command> [args] --repo-root .`
...
**Background automation (n8n):** Four workflows run automatically — always start n8n via `tools\run_n8n.bat` (sets `NODES_EXCLUDE=[]` to unlock the Execute Command node; bare `n8n start` will create workflows that refuse to activate). Dashboard: http://localhost:5678
...
| Follow-up Nudge + Dossier Freshness | Daily 9am | Writes inbox items if overdue contacts or stale dossiers |
| Weekly Review Reminder | Friday 4pm | Writes `inbox/YYYYMMDD-weekly-review-reminder.md` |

Wrapper scripts (`n8n_outreach_nudge.py`, `n8n_dossier_nudge.py`, `n8n_weekly_reminder.py`) delegate to existing preprocessing scripts and write to `inbox/` only when actionable.
