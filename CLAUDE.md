...
- **Hook-tier promotion:** add a check_*.py hook in `tools/` and wire it into `.claude/settings.json`. **For a PreToolUse hook, BLOCK (exit 2) or don't build it.** An exit-0 WARN on that surface writes to stderr, and Claude Code does not surface it — the hook detects and informs nobody, which is worse than no hook because the rule then reads as covered. If the rule is too soft-bounded to justify BLOCK, put it where stderr is read: a `Stop` hook, a pre-commit/pre-push gate, or a skill step. (Exit 0 is correct on `UserPromptSubmit`, where stdout context-injection *is* the delivery mechanism.) Per [[feedback_warn_vs_block_hook_design]] — note its pre-2026-05-28 "default to WARN exit 0" form is **superseded**; that stale phrasing sat in this bullet until 2026-08-14 and is how `gen_pii_denylist.py` shipped a WARN tier that warned into a void while a real pipeline company reached two public files. Scaffold from `tools/HOOK_AUTHORING.md` (command-position regex + quote-stripping + clean/block tests + mandatory live smoke) so the new hook doesn't repeat the command-position-not-substring blind-spot class.
