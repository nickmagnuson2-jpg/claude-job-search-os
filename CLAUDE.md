...
- **Enforcement tier: name it before building anything.** A rule is only real if it lives where it executes: **a check a gate reads, a field a schema demands, a record a run cannot skip, or a dependency the interpreter enforces.** The fourth (2026-09-07) governs CODE: "keep these N copies in sync" is prose, while an import cannot drift from its source. **It enforces NON-DRIFT, not adoption** — a new file can still write its own copy, so adoption needs its own gate. **Extract MECHANISM, never POLICY**: the test is *would every consumer want the same answer?* Yes means mechanism, so shared domain logic like stage classification IS mechanism and the rule below holds. Plan: `output/analysis/090726-consolidated-handoff.md`. Prose requiring in-moment judgment converts at zero (measured: an 18-item enumerated list, zero violations in ~80 min live, while every abstract rule failed; four written artifacts predicted a failure correctly and changed nothing on the day). Before adding any rule, guard, skill, or doc, state which of the four it is. **If the answer is "written down and followed," it is not built yet.** Upstream of every other Hard Rule: it governs what may become one. Derivation: `framework/analysis-method.md`, the conversion law.
...
- **Never infer or upgrade Nick's role titles or scope from thin context.** His McKinsey title was **Associate**, which is the management-consultant role at McKinsey (marketplace + customer-engagement transformations). Nick's own framing, 2026-09-21: *"I was an associate, which is a management consultant at McKinsey."* **Both are correct and the choice is audience-dependent** - "Associate" is the literal title and the sourced fact (`data/projects/mckinsey.md`, 10/2022-02/2024), "management consultant" describes the work and reads better to a non-consulting audience. Stating "Associate" is never an upgrade. What is banned is **"Engagement Manager"** or any implied seniority level. His ESPN role was **digital analytics manager/analyst within product analytics** (product + social/content performance) — NOT "SEO." Cite what he DID, never a level or function he didn't hold. Fired 3x on the same shape (2026-06-22 at a target company: "ran SEO at ESPN"; 2026-06-25 at another + 2026-07-07 fit-assess: both "I am not an EM") before this rule existed — one bad inference propagates into a dossier, a prep doc, and a live interview opener before Nick catches it. See `memory/reference_nick_mckinsey_role.md` and `memory/reference_nick_espn_role_is_product_analytics.md`.
- **Never assert absence without running the check this session and naming the scope.** "X doesn't exist" / "Y didn't happen" / "not found" / "no result" claims about codebase state, file contents, data records, or memory entries MUST be backed by a grep/ls/Read in the current session AND the assertion must state what was checked (e.g., "no `check_skill_shadow.py` in `tools/` per `ls`" — not bare "the hook doesn't exist"). Memory and prior-session knowledge are not sufficient — they decay. Asserting absence from memory creates ghost facts: a future audit cycle treats Claude's "not built" as authoritative and skips the verification, compounding the original error. Origin: `feedback_llm_verification_system` Rule #1 + Rule #13 (scope-limited query). **Naming the scope is not sufficient: state why that scope would have contained the thing if it existed.** Fired 2x on 2026-08-20, both WITH a named scope: a 4-file grep that omitted the file holding the rule, and an `output/` sweep for a phrase the real instrument never used (it was in `memory/lessons.md`). Family-N extends to data-state absence claims (e.g., "you haven't applied yet" — see `feedback_pipeline_applied_status_must_be_user_confirmed`, 5/28 ghost-row incident).
- **Never make a durability, backup, sync, version-control, coverage, or RETRIEVAL-CAPABILITY claim without running the function-first tooling sweep and naming it as the scope.** **"I cannot retrieve X" / "I could not verify X" / "X is not obtainable from here" is the same claim shape and is governed identically** — added 2026-09-15 after the 7th fire, which walked past this bullet precisely because the surface list did not name it: a JS-rendered Ashby page defeated one `WebFetch`, that single failure was reported as a capability limit, and a JD was then characterized from a subagent's paraphrase, while `tools/career_scanner/ashby.py` wrapped the public posting API that returns the full text and `data/job-pipeline.md` recorded the technique in prose. **One failed method is not a scope, and a hedge is not verification** — "treat this as unconfirmed" reads as rigor and is indistinguishable from the inside from having checked. Required before any retrieval claim: `ls tools/ | grep -iE "<function>"` AND `grep -rl "<vendor/technique>" tools/ .claude/skills/ data/ output/` (**prior art lives in data, not only in code**). No git command answers "is this data backed up." `git rev-parse`, `git remote -v`, `git check-ignore`, and `find -name .git` are **repo-shape** checks; they answer "what does git see from here," and the real mechanism here is an **overlay `GIT_DIR` outside the working tree** that is invisible to every one of them. Required before any such claim: `ls tools/ | grep -iE "backup|sync|push|mirror|archive"` and `grep -rl "git push\|rsync\|remote add" tools/ .claude/skills/`. `/data/` and `/output/` are gitignored **by design** (public repo) and are backed up by `tools/backup-data.sh` to a **private** repo, run nightly by `/checkout`; a retired decoy clone elsewhere on disk looks like evidence of absence and is not. **The concrete repo name, account, overlay path, decoy path, and the `gh api ... --jq '{pushed_at}'` verification command live in the gitignored `memory/reference_private_data_backup_mechanism.md`** — read it before making the claim; it is deliberately not in this public file. **Fired 3x on the same shape** (7/8, 7/30, 7/31 — the last two asserted false absence and the 7/31 fire also misdirected Nick to verify Obsidian Sync instead of surfacing that the real backup was a day stale). See `memory/feedback_prefer_extending_existing_infra_over_new_build.md` and `memory/reference_private_data_backup_mechanism.md`.
- **`grep` here skips gitignored trees. Name `data/` and `output/` explicitly, or the search is blind.** The shell's `grep` resolves to a ripgrep-backed wrapper that honors `.gitignore`. `/data/` and `/output/` are gitignored by design (public repo) and hold nearly all the real content: dossiers, reflections, generated CVs, pipeline artifacts. **A repo-root `grep -ri "<term>" .` cannot see any of it and returns an empty result indistinguishable from a true negative.** Adding `--exclude-dir=.git` makes it look deliberately scoped while changing nothing. Any search establishing presence or absence must either name the trees (`grep -ri "<term>" data/ output/`) or bypass the wrapper entirely: `find . -path ./.git -prune -o -path ./memory/canonical-sidecar -prune -o -path ./memory/global-claude-mirror -prune -o -name '*.md' -print | xargs /usr/bin/grep -l "<term>"`. **Both prunes are mandatory, not optional:** those two trees are rsync backup copies of the live memory tier (see Repository Structure above), so omitting them silently doubles every hit and makes a one-location result look like corroboration from two. **Fired 2x** (2026-08-07: a contact search returned nothing while a dossier and three output dirs existed; 2026-08-12: a sweep for a corrected-away CV overclaim returned nothing while **43 files** carried it, on a PII-relevant back-propagation task). This is the concrete meaning of "name the scope" in the absence-assertion rule above. See `memory/feedback_grep_is_ripgrep_gitignore_blindspot.md`.
...
- **A green test is not evidence. Mutation survival is.** Before claiming a test protects a behavior — in a commit message, a completion report, or to Nick — break that behavior on purpose and confirm a test dies: `PYTHONIOENCODING=utf-8 python3 tools/mutation_check.py <file> --isolation`. Exit 2 means the suite is decorative at that spot. **A surviving mutant is acceptable ONLY with a written reason in `tools/mutation-allow.json`**; an empty reason fails the run, because an allowlist without justification is how this decays back into "green means done." Run the `--isolation` half too: a test file must pass ALONE, not only inside the suite. **Fired 4x in one session (2026-08-19), every shape green at the time:** a flag deciding whether PRIVATE mail lands in this PUBLIC repo could be made a no-op with zero failures; two PII-denylist parsers could `return set()` with 20/20 passing; a guard test went vacuous when its own inputs died; and four tests failed when run alone while 2000 passed together, hiding a live `ImportError`. **A check also inherits the tolerance of its instrument.** 2026-08-20: malformed YAML was written, verified with the lenient in-house parser, and reported clean; a stricter reader caught it 20 min later. Verify with the strictest reader downstream, never the most convenient one. Writing tests that pass is not the goal — writing tests that would FAIL if the code broke is. Those are different properties and only the second is worth having.
...
- **Public-repo PII gate.** This repo is PUBLIC. **A public artifact is any file git does not ignore** — not a fixed list of directories. `tests/`, `.claude/skills/`, `.claude/workflows/`, `framework/`, `docs/`, `examples/`, `plugins/`, `tools/*.{py,md,sh}` and top-level `*.md` are the common cases, but the list has under-covered **twice** (2026-08-13 `.claude/workflows/`; 2026-08-18 `.claude/settings.json` plus an untracked `.bak` beside it that one `git add -A` would have shipped). Anything not gitignored must use generic placeholder examples, never real contacts/pipeline-target companies. Two layers: (1) `tools/check_public_pii.py` PreToolUse hook (always-on, BLOCKs known real tokens from the gitignored `tools/.pii-denylist.txt`) — **wired on `Write|Edit` AND `Bash` since 2026-08-19**; before that a heredoc or `>` redirect to a public file bypassed the gate entirely, which is the most common way a file gets written. Bash targets are parsed per command-segment, so a compound call writing clean content publicly and a real name privately is not a false positive; (2) `/audit-pii` skill — run before committing/pushing any public-file change for the semantic subagent pass that catches new names the denylist misses. Regenerate the denylist via `tools/gen_pii_denylist.py` after adding contacts/pipeline rows. Origin: 2026-06-11, `feedback_generalize_examples_in_public_artifacts`.
- **`--debug` = log this error to the friction ledger.** When Nick types `--debug` (alone, or right after something errored), append the immediately-preceding tooling failure to `memory/friction-log.md`: `PYTHONIOENCODING=utf-8 python3 tools/friction_log.py append <surface> "<nature>" --fix "<what you did instead>"`. `<surface>` is the script/hook/tool that failed (`pipe_write.py`, `check_public_pii.py`, `bash:grep`). The ledger dedupes by (surface, nature) and carries its own ladder: **1st fire logs · 2nd → write a `feedback_*.md` · 3rd → mandatory script patch.** Then tell Nick the count and the ladder rung. **Scope: TOOLING friction only** — a script that rejected valid input, a hook false-positive, a missing subcommand, a usage error that burned a turn. **Claude's own reasoning/inference errors do NOT go here**; those route to the memory corpus per the Self-Improvement Loop below. Mixing them makes the ladder meaningless, because a reasoning error has no `surface` to patch. Origin 2026-08-14: the ledger already existed with 254 lines and was referenced nowhere auto-loaded, so it never fired.
...
After any user correction, run the **full tiered protocol** — not just a lessons.md row. The tier ladder (per [[feedback_llm_self_policing_fails]]: memory < skill < hook < independent reviewer) determines *where* the rule lives; the steps below determine *how* it gets there.

### Step 1 — Capture at memory tier (always)

1. **Create** `~/.claude/projects/-Users-mag-Documents-Obsidian-30-projects-job-search/memory/feedback_<kebab-case-slug>.md`. **All six `metadata` keys are mandatory for `type: feedback`:**
   ...
   metadata:
     ...
     reopen_gate: "<trigger + target; MUST name a number ('3rd fire'), never 'the next dated fire'>"
     last_cited: YYYY-MM-DD
     promoted_date: YYYY-MM-DD   # these 3 set on exit only; promotion_schema.py BLOCKS
     exit_path: exit1            # exit1 | exit2 | terminal
     detector_signature: "..."   # REQUIRED for exit2
   ---
   ```

   **`occurrences` / `promoted` / `reopen_gate` / `last_cited` are the ONLY keys `tools/scan_promotion_candidates.py` reads** — the detector behind both `/memory-refresh` and the weekly `memory-promotion-scan` job. A memory file without them is **invisible** to it: the rule accumulates fires while reporting as a first-timer, and no promotion ever surfaces. Prose alone does not register. (384 of 403 `feedback_*.md` files, 95%, were invisible this way until it was fixed 2026-08-13.)
...
2. **If the correction refines an existing rule**, update that file with a dated supplement section instead of creating a duplicate.
3. **Do NOT write an index entry.** The corpus is a **ledger** queried by `grep`. The 11 `index-<topic>.md` shards are **frozen 2026-08-25: archived, never deleted.** The rule file IS the record. Write to MEMORY.md only for `exit_path: exit2`.
4. **For email/outreach corrections specifically:** also add a row to `memory/lessons.md` Section 2 (Occurrences=1, Promoted=No). Recurring → increment Occurrences. If Occurrences ≥ 2 and Promoted=No → update `framework/style-guidelines.md` "Nick's Voice" section and set Promoted=Yes. This email-specific path does NOT replace the auto-memory file.
...
**Two exits, NOT equal.** **`exit1`** (BLOCKing hook / schema field / check) is verifiable and **must be tried first**; a PRESENCE-only check is not a valid exit1. **`exit2`** (always-loaded) is **the residual**, priced by a `detector_signature` (**never waived**; validate via `tools/detector_probe.py`), an eviction, and a probation review. `promotion_schema.py` BLOCKS on all three. Detail: `output/analysis/082526-self-learning-loop-plan.md`.

Record `promoted_date` + `exit_path`, or the promotion is unobservable and the drain rate cannot be measured.

- **Trace to source before building a guard (mandatory).** Before adding a hook or any forward-guard, ask: *what artifact or instruction generated this bad input?* If a doc, skill, template, or printed-help string prescribes the failing pattern, fix THAT in the same pass. A forward-guard over a poisoned source leaves the recurrence intact — the guard just trips on every run while the source keeps emitting the bad pattern. (2026-06-02: `check_bare_python.py` was built to block bare `python` while 13 skill docs + tool docstrings still prescribed it.)
- **Back-propagate to existing artifacts (mandatory for greppable rules).** A newly-learned rule is enforced FORWARD by default; the artifacts that ALREADY violate it must be swept in the same pass. When the rule has a greppable violation signature, run `PYTHONIOENCODING=utf-8 python3 tools/audit_rule_violations.py --pattern '<signature>'` and sweep or surface every existing hit. Skipping this is how a rule sits "captured" for weeks while broken artifacts accumulate.
- **Name the target surface AND when Nick last ran it (mandatory before any skill-tier promotion).** A rule anchored to a surface that never fires is **worse than an unpromoted rule**: the promotion flips `promoted: yes`, the rule leaves the backlog, and the detector stops surfacing it. Rank surfaces by whether they need Nick at all: **launchd job > hook > CLAUDE.md (auto-loaded) > `/standup` > a skill he invokes occasionally > a skill he rarely invokes**. `/standup` yes; `/weekly-review` and `/memory-refresh` no. If the only sensible home is a rarely-run skill, land it there but say so plainly and do NOT treat it as closed. (2026-08-13: two promotions landed on `/weekly-review` and `/memory-refresh` hours before Nick said he doesn't run them — correct content, dead surface.)
- **Skill-tier promotion:** edit the relevant skill SKILL.md so the rule is a mandatory step, not implicit context.
- **Hook-tier promotion:** add a `check_*.py` hook in `tools/` and wire it into `.claude/settings.json`. **PreToolUse: BLOCK (exit 2) or don't build it** — an exit-0 WARN goes to stderr, which Claude Code never surfaces. Too soft for BLOCK? **`UserPromptSubmit` is the fourth option and the only one where a non-blocking hook still reaches the model** — stdout IS the injection channel there, so exit 0 delivers — but **none is wired in any layer** (2026-09-15); the one attempt, `check_status_query_verification.py`, is built and **deliberately unwired** on precision. Otherwise a `Stop` hook, pre-commit, or a skill step. **Wiring any gate takes TWO measurements — restraint (replay real history) AND mutation survival — which fail independently.** Per [[feedback_warn_vs_block_hook_design]] (its "default to WARN" form is **superseded**). Scaffold from `tools/HOOK_AUTHORING.md` (command-position regex + quote-stripping + clean/block tests + mandatory live smoke) so the new hook doesn't repeat the command-position-not-substring blind-spot class.
- **Framework-tier promotion:** for tonal/voice rules with Occurrences ≥ 2, update `framework/style-guidelines.md` "Nick's Voice" section (the email/outreach path above).
- **Hard-rule promotion:** for project-tier rules unmissable in any session, add a bullet to the `## Hard Rules` section of this file. Reserve for rules that meet the bar: structural, unambiguous, and bypassable by skill-tier defects.

### Step 4 — Scan on relevant sessions

Scan `memory/MEMORY.md` (auto-loaded every session — Critical Context + the Topic Shards router) at the start of relevant work, and `memory/lessons.md` on skill-edit / data-ops / CV-gen sessions. For anything beyond Critical Context, load the relevant `memory/index-<topic>.md` shard(s) named in the router table — that is where the actual index entries live, not in MEMORY.md itself. Full memory files load on demand via recall.

**Origin incidents behind these steps, plus the anti-pattern list** → `docs/self-improvement-loop.md`. Read it when you want the incident a step came from, when deciding whether a step applies to an edge case, or when auditing whether this loop is working. **That file contains no directives and does not restate this protocol** — the procedure lives here and only here, so there is nothing to keep in sync. The pre-2026-08-18 verbatim text is in git (`git show 67e0ada:CLAUDE.md`).
...
memory/            ⚠️ THREE PHYSICAL ROOTS, not one. Resolve before asserting any file is absent.
  ...
  │                    Holds lessons.md, friction-log.md, archive-2026-04-orphans.md. Gitignored.
  └─ <repo>/memory/canonical-sidecar/ + global-claude-mirror/
                       BACKUP DESTINATIONS ONLY, rsync'd by tools/backup-data.sh so the
                       out-of-repo live tier reaches the nightly private-repo backup.
                       NEVER search these: they duplicate every hit. NEVER delete them:
                       they are required for backup coverage.
...
**Do not write a session handoff. Cross-session state goes to `data/workstreams/`.** 22 accumulated in `output/analysis/` + flat `output/`, each with an invented format and no disposition, so no reader could tell which was authoritative; one landed the day the registry replacing them shipped. **A workstream needs a decision open across sessions AND state a future session would re-derive** — only one, and it is a todo (`tools/todo_write.py`); derivable numbers go in a `PROBE`, never typed. `tools/check_handoff_producer.py` (PreToolUse) BLOCKS **creating** such a `.md` **in those two dirs only**. **The rule is broader than the gate**: `output/<slug>/` is ungated, and Bash bypasses exist (`cd x && >f`, `cp -t`, `touch`) incl. stripping a banner (3 P0 open, 2026-09-15). A Write-path backstop, not proof. The 22 carry a `<!-- DISPOSITION: {...} -->` banner enumerated by `tests/scripts/test_handoff_disposition.py`; **`live`/`reference` ones remain authoritative** — read the banner before trusting or retiring one.
...
**All `tools/*.py` scripts require a `PYTHONIOENCODING=utf-8` prefix or they crash on Unicode.** Python 3.10+.

**Environment detail, tool tables, and per-workflow recipes** → `docs/tools-reference.md`. Read **before invoking any `tools/*.py` script** (argument order, flag placement, `--repo-root` position), before touching `tools/launchd/`, before writing an email draft or rendering a CV PDF, after an interview, or when a gitignored private config file appears to be missing. Contains: atomic write scripts, launchd background jobs, private local config, multi-agent workflow templates, the RenderCV pipeline, the `.pending-draft.txt` format, the post-interview 6-step workflow, and the Python/PEP-604 version constraint.

**Gotcha that must never depend on recall:**
...
**MEMORY.md is loaded every conversation and is the ONLY channel measured to reach the model.** Its bytes are the scarce resource: **admission requires eviction.** It holds two things only — Critical Context, and `exit2` principles (Loop Step 3).

**Keep in MEMORY.md's Critical Context block (small, always-visible, never depend on recall):** employment status, family contacts, active hard-rule-DUE items. Plus `exit2` principles, each carrying a `detector_signature` and a probation review date. Nothing else belongs here: active search context, architectural patterns, unfixed bugs, and user preferences live in their own rule file in the live tier and are found by `grep`, not by an index.
