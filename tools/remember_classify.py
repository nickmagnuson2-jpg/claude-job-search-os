...
Output JSON (stdout):
  {
    "note": "...",
    "destinations": [
      {"type": "contact_note", "file": "data/networking.md", "entity": "Jordan Lee",
       "matched_name": "Jordan Lee"},
      {"type": "company_note", "file": "data/company-notes/acme-ai.md",
       "entity": "Acme AI", "slug": "acme-ai"}
    ],
    "ambiguous": false,
    "warnings": []
  }
...
Usage:
  PYTHONIOENCODING=utf-8 python3 tools/remember_classify.py --note "text" [--repo-root PATH]
"""
import argparse
import json
import re
...
PROFILE_KEYWORDS = [
    "compensation", "salary", "availability", "start date",
...
ACCOMPLISHMENT_KEYWORDS = [
    "shipped", "landed", "completed", "finished", "accomplished",
    "milestone", "made progress", "nailed it", "achieved", "pulled off",
]
...
def classify_note(
    ...
    # ── 6. Decision ───────────────────────────────────────────────────────
    # Decision runs before accomplishment: explicit "decided/won't/going with"
    # is more specific than the broader accomplishment win-words.
    if _contains_any(note, DECISION_KEYWORDS):
        return {
            "note": note,
            "destinations": [{
                "type": "decision",
                "file": "data/decisions.md",
                "entity": None,
            }],
            "ambiguous": False,
            "warnings":  warnings,
    ...
    # ── 6b. Accomplishment ────────────────────────────────────────────────
    if _contains_any(note, ACCOMPLISHMENT_KEYWORDS):
        return {
            "note": note,
            "destinations": [{
                "type": "accomplishment",
