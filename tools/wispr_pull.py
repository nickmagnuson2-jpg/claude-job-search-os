...
Destination resolution:
  1. ~/Documents/Obsidian/00-voice-corpus-archive/  (vault, once Phase 1 migration runs)
...
def main() -> int:
    ...
    p.add_argument("--print", dest="print_content", action="store_true",
                   help="Also print transcript content to stdout (chat-friendly). Files still written.")
    args = p.parse_args()
    ...
    if args.all:
        ...
        m = re.match(r"^(\d+)([dhm])$", args.since)
        if not m:
            sys.exit("--since must look like '30m', '7d', or '24h'")
    ...
    written, skipped = [], []
    for n in notes:
        # --all bypasses the imported_ids check. Plain runs still skip already-imported.
        if not args.all and n["id"] in state.get("imported_ids", []):
        ...
        # --all + --dry-run is a preview mode; allow file-exists pass-through so
        # transcripts surface even if the archive already has a copy.
        if path.exists() and not (args.all and args.dry_run):
    ...
    if args.print_content and written:
        print()
        print("=" * 60)
        print("TRANSCRIPTS")
        print("=" * 60)
        for n, _path, _status in written:
            created = parse_wispr_ts(n["createdAt"])
            print()
            print(f"--- {created.strftime('%Y-%m-%d %H:%M')} | {derive_title(n)[:60]}")
            print()
            print(n["content"].strip())
        print()
