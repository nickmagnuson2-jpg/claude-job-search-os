...
**Customizing the rendered PDF:** CV PDFs are produced with [RenderCV](https://github.com/rendercv/rendercv) from a content YAML composed against a visual theme in `framework/cv-themes/` (default `tuck-mbb.yaml`). The regional formats above govern *content and section conventions*; the theme YAML governs the *typography, spacing, and layout* of the rendered PDF. To change the look, edit or add a theme in `framework/cv-themes/` and point the render step at it; to change content conventions, edit `style-guidelines.md` as above. See `/generate-cv` and `/apply` for the full render pipeline.
