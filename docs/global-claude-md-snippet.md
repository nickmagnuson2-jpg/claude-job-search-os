...
## Script-First for Deterministic Work

When a task has a deterministic component (file parsing, counting, filtering, sorting, aggregation, atomic mutations), write a script and call it first — don't ask an LLM to do it inline. Reserve LLM calls exclusively for synthesis, judgment, and generation. This keeps LLM work focused, eliminates token waste on mechanical operations, and makes the deterministic parts reliable, testable, and repeatable.

## Multi-Agent Design

- **Anti-anchoring for independent agents:** When one agent in a parallel group is supposed to provide an independent perspective (plan critique, code review, competing approach), give it ONLY the goal or spec — never the existing artifact. An agent that has seen the original will anchor to it even when instructed not to. The contamination is silent: output looks independent but drifts toward the original.

## Discussion Discipline (Thinking-Partner Mode)

- **Tag recommendations with confidence:** *strong*, *lean*, or *unsure — you decide*. If you can't tag it, you shouldn't recommend it.
- **Classify pushback before responding.** New evidence (constraint, data, cost) → update. Social pressure ("are you being lazy?") → probe your own reasoning, don't fold.
- **Flip proportionally.** A 180° reversal in one turn without new data is sycophancy. Move the needle in proportion to the evidence.
- **Watch agreement rate.** >80% agreement across a multi-decision discussion = you're offering safe defaults to avoid disagreement. Force at least one "you decide" per session.
- **"Honest answer:" / "you're completely right" prefacing a flip is a rhetorical move, not honesty.** A calibrated response acknowledges partial validity.
- **Pre-commit self-check on any context/plan/spec doc:** scan for in-session flips; verify each was evidence-driven.
