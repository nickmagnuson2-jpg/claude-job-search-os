...
def test_fresh_dossier_detected(tmp_path):
    ...
    make_dossier(tmp_path, "acme-ai", "2026-02-24")
    ...
    assert "acme-ai" in slugs_this_week
...
def test_stale_dossier_alert(tmp_path):
    ...
    The default now only alerts on dossiers whose company is actively pipelined
    (is_target_dossier, added 2026-06-02 to silence dead-lane noise). This test
    exercises the date-threshold logic itself, so it passes --all-stale to opt
    into the legacy alert-on-all behavior.
    """
...
def test_non_dossier_files_excluded(tmp_path):
    ...
    make_dossier(tmp_path, "acme-ai", "2026-02-24")
    make_non_dossier(tmp_path, "acme-ai", "022426-chief-of-staff.md")
    make_non_dossier(tmp_path, "acme-ai", "022426-prep.md")

    result = run_script("dossier_freshness.py",
                        "--target-date", "2026-02-26",
                        "--repo-root", str(tmp_path))

    # Only acme-ai.md should be detected, not the CV or prep files
