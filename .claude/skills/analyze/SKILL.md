...
Read Nick's context first: `CLAUDE.md` always (the architecture: skills, hooks, scripts, memory tiers, data model), plus as relevant `data/goals.md`, `data/professional-identity.md`, and `MEMORY.md`. MEMORY.md itself now holds only Critical Context + a Topic Shards router (11 shards since the 2026-08-13 split) — for systems-fit judgment specifically, also skim the router table and read whichever `memory/index-<topic>.md` shard(s) look most relevant (`index-tools.md`, `index-agents.md`, and `index-verification.md` are the most likely to matter for "does this map to something in the OS"). Then produce, from the `ExtractionBlock`, three co-equal reads:
