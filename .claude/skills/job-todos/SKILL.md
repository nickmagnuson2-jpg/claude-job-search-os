...
Examples:
- `/job-todos` — show active to-dos sorted by priority, with pipeline/contact links
...
- `/job-todos daily` — generate and archive today's progress summary
...
> Auto-generated by `/job-todos daily`. One entry per day.
...
### To-Do List (shown by `/job-todos` with no args)
