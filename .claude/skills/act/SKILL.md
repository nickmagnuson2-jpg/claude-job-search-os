...
argument-hint: [none]
user-invocable: true
allowed-tools: Read(*), Write(data/job-todos.md), Write(output/**), mcp__exa__web_search_exa, mcp__exa__web_fetch_exa, WebFetch(*), WebSearch, Task, Glob(output/**), Glob(inbox/*), Bash(PYTHONIOENCODING=utf-8 python3 tools/act_apply.py:*), Bash(PYTHONIOENCODING=utf-8 python3 tools/act_classify.py:*), Bash(PYTHONIOENCODING=utf-8 python3 tools/todo_write.py:*), Bash(rm inbox/*)
---
...
```bash
PYTHONIOENCODING=utf-8 python3 tools/act_classify.py
```
...
**Stale items override (mandatory):** Items with `stale: true` in the classification — regardless of `type` or `source_type` — are NEVER queued for routing or approval. They go in a dedicated "Stale Inbox — Will Auto-Delete" section of the preview and are deleted on `run` without per-item approval.

`act_classify.py` detects three stale cases:
1. `sender_already_in_networking` — Gmail sender's email is already in `data/networking.md` (contact was already logged; re-routing would duplicate)
2. `company_already_in_pipeline` — ATS company slug from a single-listing inbox file already has a pipeline entry
3. `superseded_by_newer_nudge` — older copy of a daily launchd nudge (dossier-freshness-alert / follow-up-nudge / weekly-review-reminder); only the newest is canonical

The check is deterministic and read-only — safe to auto-delete on `run`. If a stale item is somehow misclassified (false positive), the inbox file can be regenerated by the upstream source.

Origin: 2026-05-14 contact-at-target-company case — inbox file from 5/12 was routed manually via `/networking-write` + `/draft-email` (without `/act`), so the file stayed in `inbox/`; `/act` re-classified it as a fresh contact_capture and queued a duplicate write. Per memory [[feedback_search_destination_folder_first]] + [[feedback_skill_workflows_close_trigger_todos]].

### Step 1b: Read Manifest (mandatory before preview synthesis)

Before producing the preview in Step 3 (especially the inbox routing decisions and any "what the inbox contains" claims), emit a Read manifest as an internal-check step:

```
## Read manifest
**Files Read in full this session:**
- [each file path actually called via Read tool — usually data/profile.md plus any inbox files whose content you're claiming knowledge of]

**Files referenced by grep/snippet/filename only (NOT Read in full):**
- [list, or "(none)"]
```

**Rule:** Do not make content claims about inbox files (e.g., "this article covers X," "this email is about Y") from filename or `content` JSON snippets alone. The `act_classify.py` output includes a `content` field that is *partial* — if a routing decision depends on the FULL content of an inbox file, Read it before claiming. Partial reads count as "snippet only," not "full Read."

This is especially load-bearing for Gmail items, where misclassification can result in a wrong write to networking/pipeline/notes. Read the full file before approving any Gmail item routing.

This manifest is internal-check; do not surface in the preview unless the user asks.

Origin: [[feedback_no_confabulation_in_corpus_synthesis]] (Conviction Workbook Part 1 audit 2026-05-12).
...
- `inbox_items` where `stale: true` → "Stale Inbox — Will Auto-Delete" section (grouped by `stale_reason`, count + sample filenames)
- `inbox_items` where `stale != true` AND type in {job_ad, contact_capture, unclassifiable} AND `source_type != "gmail"` → "Inbox — Immediate Routes" table
- `inbox_items` where `stale != true` AND `source_type == "gmail"` AND type in {job_ad, contact_capture, unclassifiable} → "Gmail Emails — Needs Approval" table (after Immediate Routes section; shown even if Immediate Routes is empty)

**Stale exclusion rule:** Any item with `stale: true` is excluded from Bucket A, Immediate Routes, and Gmail Approval. It appears ONLY in the auto-delete section.
...
| 8 | Company research | (inbox) Northwind | 5 agents + dossier |
...
- **Acme AI** — dossier from 2026-02-18 (`output/acme-ai/acme-ai.md`)
...
| 10 | Follow Jordan Lee, Alex Rivera on LinkedIn | LinkedIn search |
...
|---|------|------|---------|------|----------------|
| G1 | Jordan Lee <jordan@example.com> | Jan 8 | Nick / intro | Contact | → Add to networking.md |
| G2 | jobs@greenhouse.io | Mar 17 | Your application | Job ad | → Pipeline: Acme AI · Researching |
...
### 🗑️ Stale Inbox — Will Auto-Delete on `run`
Detected by `act_classify.py` cross-checking destination data files + supersedence patterns. No approval required — these are confirmed-stale by deterministic check.

| Reason | Count | Sample filenames |
|---|---|---|
| `superseded_by_newer_nudge` | 23 | 20260429-dossier-freshness-alert.md, 20260505-follow-up-nudge.md, … |
| `sender_already_in_networking` | 1 | 20260512-120224-jordan-lee-role-at-target-worth-20-mins.md (sender already logged in networking.md) |
| `company_already_in_pipeline` | 0 | — |

> Skip this section entirely if no stale items.
...
| # | File | Content | Action |
|---|------|---------|--------|
| I3 | greenhouse-job.md | "https://job-boards.greenhouse.io/acme-ai/jobs/123" | → New pipeline entry: Acme AI \| Researching |
| I4 | sarah-chen.md | "met Sarah Chen at SF health event, head of ops at Northwind" | → New contact: Sarah Chen \| Northwind \| Head of Ops |
...
**Stale auto-delete (mandatory, runs on `run` / `run [numbers]` / `run fast` / `run research` — anything that isn't `cancel`):** For each item in `inbox_items` with `stale: true`, run `Bash(rm inbox/[filename])` in parallel with the regular execution. No per-item approval required — staleness was detected deterministically by `act_classify.py`. Report the count under "Inbox Routed → Stale" in the results section. On `cancel`, do NOT auto-delete stale files (the whole run is aborted).
...
|---|------|---------|----------------|
| G1 | Jordan Lee <jordan@example.com> | Nick / intro | → Add to networking.md |
| G2 | jobs@greenhouse.io | Your application | → Pipeline: Acme AI · Researching |
...
```
Run a full company research dossier for [Company Name] using parallel research. Per the 2026-05-19 A/B verdict, the 5 sub-agents use Exa MCP as primary retrieval (mcp__exa__web_search_exa, mcp__exa__web_fetch_exa); WebSearch/WebFetch only as fallback if Exa returns nothing on a query.

You have access to: mcp__exa__web_search_exa, mcp__exa__web_fetch_exa, WebSearch, WebFetch, Read, Write, Task, Glob.

Research process:
1. Launch 5 parallel sub-agents (using Task tool, model: "haiku", max_turns: 8) — each agent must use Exa MCP as primary retrieval. Covering:
...
URL: [URL from notes]
Article context: [task text — e.g., "Jordan Lee's white paper on care delivery and care-retention"]
...
2. Identify the PRIMARY company this article is about. Look for:
   - A single company named as the subject (e.g., "Acme AI announces...", "Jordan Lee from Acme AI")
...
4. Save the full summary to a file:
   - If primary_company is set: output/[slug]/[MMDDYY]-article-[article-slug].md
     where [slug] = company name lowercased, hyphens for spaces (e.g., "acme-ai")
...
**Jordan Lee — Care Delivery and care-retention Treatment** (an industry publication)
- Digital health interventions reduce care-retention dropout by 40% in pilot studies
- Insurance parity laws are expanding reimbursement for digital therapeutics
- [3 more key insights]
- Why it matters: Provides credibility talking points for the Acme AI coffee chat with Jordan.
> Summary saved: `output/acme-ai/022426-article-oud-treatment-digital-health.md`
...
- Acme AI job ad → pipeline
- Sarah Chen → new contact added to `data/networking.md`
- an industry publication article → executed as article read (see Articles Read above)
- Northwind → executed as company research (see Company Research above)

**Gmail (approved by you):**
- G1: Jordan Lee intro → contact added to `data/networking.md`
- G2: jobs@greenhouse.io → Pipeline: Acme AI · Researching
...
| Follow Jordan Lee on LinkedIn | [linkedin.com/in/...] |
| Follow Alex Rivera on LinkedIn | [linkedin.com/in/...] |
