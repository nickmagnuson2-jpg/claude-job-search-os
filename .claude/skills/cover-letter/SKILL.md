...
user-invocable: true
allowed-tools: Read(*), Glob(data/*), Glob(framework/*), Write(output/**), Write(data/job-pipeline.md), mcp__exa__web_search_exa, mcp__exa__web_fetch_exa, WebFetch, WebSearch
---
...
- **`[context]`** (optional) — e.g. `"emphasize the Acme work"`, `"more informal tone"`, `"keep to 200 words"`, `"mention coffee chat with Jordan"`

Examples:
- `/cover-letter https://company.com/jobs/cos` — fetch posting, generate letter
- `/cover-letter "Chief of Staff, Northwind..." "emphasize food/FMCG"` — pasted JD with context
- `/cover-letter https://jobs.lever.co/acme-ai/xyz "warm tone, mention coffee chat with Jordan"`
...
- **The company's core challenge** — what specific problem is this role hired to solve? Read the JD carefully: look for language about growth, scaling, transformation, launches, or pain points. If the JD is vague, use the company dossier or a quick search via `mcp__exa__web_search_exa` (Exa primary per the 2026-05-19 A/B verdict; WebSearch only as fallback) to identify a concrete challenge.
...
Label the provenance of every substantive sentence in the letter before the quality gates.

**Substantive sentences in a cover letter:** thesis / their-problem statement / proof-point framing / self-positioning / bridge / closing CTA. Salutation and sign-off are not substantive; skip them.
...
If any `G` blocks fire, return to Step 5 and request the spine for those slots. Do not proceed to Step 6 with `G` in any blocked slot. Trace every `I` to its source before Step 6.
...
- **Sounds like Nick, not polished-generic Claude.** Read it cold: does the sentence-level voice match `voice-reference.md` (compound `and`/`but` clauses, ` - ` inline hyphen, `particularly` emphasis, no AI-smoothing), or has it flattened into competent-but-anyone prose? If the latter, rework at sentence level against the voice-reference patterns — a cover letter is a different register from email, but the voice signature still carries.
...
### Step 9b: Render PDF (mandatory terminal artifact)

After saving the markdown in Step 9, render a printable PDF for analog reading and easy attachment. Per `memory/feedback_print_prep_pdfs`.

```bash
PYTHONIOENCODING=utf-8 python3 tools/md_to_pdf_doc.py output/<company-slug>/MMDDYY-cover-letter.md
```

Produces `output/<company-slug>/MMDDYY-cover-letter.pdf`. The PDF is also useful as an application-ready attachment (some ATS portals accept PDF cover letters).

Then offer print and Finder reveal:

> Generated `output/<company-slug>/MMDDYY-cover-letter.pdf`. Run `open -R output/<company-slug>/MMDDYY-cover-letter.pdf` to reveal in Finder (per memory/feedback_finder_open_for_attachment, drag-from-Finder is your application workflow).

Do NOT auto-open without Nick's confirmation.
