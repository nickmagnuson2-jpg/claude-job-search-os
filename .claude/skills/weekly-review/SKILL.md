---
name: weekly-review
description: Weekly job search retrospective — pipeline health, outreach activity, task velocity, and ranked priorities for the coming week
argument-hint: ""
user-invocable: true
allowed-tools: Read(*), Glob(data/*), Write(data/weekly-review-log.md), Edit(data/weekly-review-log.md), Bash(python3 tools/todo_daily_metrics.py:*), Bash(python3 tools/pipeline_staleness.py:*), Bash(python3 tools/dossier_freshness.py:*), Bash(python3 tools/outreach_pending.py:*)
---
...
```bash
python3 tools/pipeline_staleness.py --target-date $(date +%Y-%m-%d) --days-threshold 7
python3 tools/outreach_pending.py --target-date $(date +%Y-%m-%d) --lookback-days 7
python3 tools/dossier_freshness.py --target-date $(date +%Y-%m-%d) --lookback-days 7
```
...
3. `data/weekly-review-agenda.md` (if it exists) — ad-hoc agenda items captured mid-week. Surface these as a dedicated **"Ad-Hoc Agenda"** section in the review output (after Pipeline Health, before priorities). After addressing each item, move it from the agenda file into the new weekly-review-log.md entry under that week's header, then leave the agenda file empty (or with just the schema/header). Skip silently if file is missing.
4. `data/decisions.md` + `data/accomplishments.md` (if they exist) — the chronological strategic-decision and process-win logs. Use `decisions.md` to frame "where the search stands and why" in the retrospective. Surface `accomplishments.md` entries dated within the review window (past 7 days) as a dedicated **"Wins This Week"** beat, and use them as substrate for any LinkedIn-post or "what worked" retro suggestion. Skip silently if missing.
5. `data/reflections/_longitudinal.md` (if present) — read the TOP entry only (newest). Surface 1-2 headline lines from its three axes (thinking patterns / decision biases / communication tendencies) as the longitudinal read in the retrospective. This is the `/my-world` deep-pass output; skip silently if the file does not exist.
...
Before producing any content claim in Steps 2-8, emit a Read manifest as an internal-check step:
...
**Rule:** Do not make content claims about any file in the second list. If a section of the review requires content from a not-yet-read file (e.g., longitudinal patterns across reflections, themes from networking entries), Read it first. Partial reads (head, offset+limit, grep snippets) count as "snippet only," not "full Read."

This manifest is internal-check; do not surface it in the user-facing review unless asked. But if any content claim cites a file from the second list, the review must be revised before output.

Origin: [[feedback_no_confabulation_in_corpus_synthesis]] (Conviction Workbook Part 1 audit 2026-05-12). Weekly-review is high-risk for this failure mode because it spans many files and tempts synthesis from snippets.
...
4. **Awaiting response overdue** — `awaiting_response_overdue[]` — each entry has: `name`, `company`, `days_since_sent`

Do NOT compute or display a response-rate percentage (`recent_outreach.response_rate_percent`). Nick doesn't track it (decided 2026-06-15); raw sent/replied/no-reply counts are the outreach signal.
...
Also ask Nick directly: "Does the ledger feel complete vs. what you remember hitting this week?" — missed-logging is the failure mode the audit catches.
...
Examples:
- "Apply to Beacon CoS role — posting may close, been researching 5 days"
- "Follow up with Jordan Lee — no reply in 6 business days"
- "Run `/research-company 'Northwind'` — pipeline entry with no dossier, been stalled 9 days"
- "Complete and send Acme AI application — stuck at Researching since Feb 18"
...
**Completed: N tasks** | Outreach: N sent, N replied | Pipeline: N active (N stalled)

#### Pipeline Health
| Company | Role | Stage | Days Since Update | Status |
|---------|------|-------|------------------|--------|
| Northwind | Chief of Staff | Researching | 9 | ⚠️ stalled |
| Acme AI | Strategy & Ops | Researching | 5 | ⚠️ stalled |
| Beacon | CoS | Researching | 0 | Active |

#### Outreach Activity
- Sent: N | Replied: N | No reply: N
