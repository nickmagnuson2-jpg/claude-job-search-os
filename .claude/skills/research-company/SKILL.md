...
description: Deep company research with parallel agents — overview, funding, people, news, competitive landscape, Exa primary + web cross-check — with conversation starters and similar companies to target
argument-hint: <company-name> [url] [context]
user-invocable: true
allowed-tools: Read(*), Glob(data/*), Glob(output/**), Grep(data/*), Write(output/**), Edit(output/**), Write(data/job-todos.md), Read(data/job-todos.md), Task, mcp__exa__web_search_exa, mcp__exa__web_fetch_exa, WebFetch, WebSearch
---

# Research Company — Deep Dossier with Parallel Agents

Research a company in depth by launching six parallel agents, each investigating a different dimension. Produces a comprehensive dossier with conversation starters tailored to the context (coffee chat, interview, networking), plus a curated list of similar companies worth targeting in your job search.
...
Examples:
- `/research-company "Acme AI" https://acme-ai.com "coffee chat with Jordan Lee"`
- `/research-company Stripe https://stripe.com "interview for PM role"`
- `/research-company "McKinsey" "networking event next week"`
- `/research-company "Acme AI"` — general research, no specific context
...
2. **URL** (optional): Any token containing `http` or matching a domain pattern (e.g., `acme-ai.com`). If a bare domain is provided, prepend `https://`.
...
**Extract contact name** from the context string if present (e.g., "coffee chat with Jordan Lee" → contact name is "Jordan Lee"). Look for patterns like "with [Name]", "meeting [Name]", "[Name] at [Company]".

**Generate a slug** from the company name for the output filename: lowercase, spaces/special chars replaced with hyphens (e.g., "Acme AI" → `acme-ai`).
...
Use the Task tool to launch **six parallel subagents** (`subagent_type: "general-purpose"`, `model: "haiku"`, `max_turns: 20`). **Agents 1-5 use Exa MCP as primary retrieval** (`mcp__exa__web_search_exa`, `mcp__exa__web_fetch_exa`); **Agent 6 is the web cross-check** using WebSearch/WebFetch only. This pattern is the post-2026-05-19 A/B verdict: Exa decisively beats web on primary-source reach (company eng blogs, live job posts, paywalled coaching, Glassdoor first-person), but keeping one web agent preserves source diversity (anti-monoculture per [[feedback_research_source_diversity_not_monoculture]]) and creates an independent corpus the synthesis step can diff for contradictions and coverage gaps.

**Model tiering — explicit by design:**
- **The 6 parallel research agents run on Haiku.** Right tool for breadth: cheap, fast, parallelizable. Their job is gather-and-cite, not reason-across-sources.
- **Synthesis (Step 4) and Executive Summary writing (Step 8) stay on the orchestrator's model (Opus/Sonnet 4.x).** This is where cross-agent contradiction detection, BLUF authoring, and rubric-application happen — the reasoning that Haiku underperforms on. The orchestrator must not delegate Step 4 or Step 8 to a sub-agent.
- **`max_turns: 20` is intentional.** Earlier versions used 12; the competitive-landscape agent in particular routinely needs 15-20 tool calls to verify careers pages and triangulate across aggregator sources. Lower caps throttle the most useful agent.
...
**Important:** Tell Agents 1-5 to use the Exa MCP tools (`mcp__exa__web_search_exa` for search, `mcp__exa__web_fetch_exa` for fetching page content). Agent 6 uses WebSearch + WebFetch as the cross-check (it must NOT use Exa — that's the whole point of having an independent retrieval). Each agent should perform 3-5 targeted searches and follow the most promising results. For Exa queries, write them as semantically rich descriptions of the ideal page (e.g., "blog post explaining [Company]'s business model"), not bare keywords — that's how Exa's neural search performs best.
...
1. **Company-adjacency (company shape):** similar problem/customer/stage to the anchor company
2. **Candidate-adjacency (role shape):** the company's *actual current hiring* (not just headcount/growth) includes the candidate's target role types. Check the careers page. If a company is structurally similar to the anchor but its open roles are all Sales / Engineering / Marketing with zero BizOps/Strategy/Ops/CoS seats, it FAILS candidate-adjacency regardless of company fit.
3. **Hiring signals (verified):** active job postings in the candidate's target role types, NOT just growth indicators or "we're hiring" claims
4. **Geographic fit:** near candidate or remote-friendly per candidate context
5. **Stage/culture fit:** matches candidate's stated stage preference per candidate context

**A shortlisted company must pass BOTH company-adjacency AND candidate-adjacency.** If a company is highly adjacent to the anchor but has no role-shape match in current openings, exclude it from the shortlist — or include it with an explicit caveat: "Company fit is strong but no candidate-shape roles currently open; check back in [N] months."
...
#### Agent 6: Web Cross-Check (Independent Source)

**Focus:** Re-cover the same ground as Agents 1-4 (overview, funding, leadership, recent news/risks) using WebSearch + WebFetch instead of Exa. Purpose is twofold: (1) source diversity against an Exa monoculture, (2) provide an independent corpus the synthesis step can diff against the Exa agents for contradiction detection and coverage gaps. Pattern flipped 2026-05-19 from the prior Exa-cross-check design after the controlled A/B promoted Exa to primary; web stays in the ensemble for diversity, not as primary.

**This agent does NOT use Exa.** It uses only WebSearch and WebFetch. This is enforced by prompt; if Agent 6 uses Exa, this run is contaminated and the cross-check loses its function.

**Research instructions:**
```
You research [Company Name] using ONLY WebSearch and WebFetch. Do NOT use any mcp__exa__* tool. Your purpose is to be an independent corpus the orchestrator can diff against the five Exa-using agents (Agents 1-5) for contradictions, missing sources, and coverage gaps. Run these searches:

1. WebSearch: "[Company] what they do products business model"
2. WebSearch: "[Company] funding round valuation investors"
3. WebSearch: "[Company] founders leadership team executives"
4. WebSearch: "[Company] news announcement 2026"
5. WebSearch: "[Company] layoffs lawsuit controversy risk"

Use WebFetch on the most promising 3-5 URLs from these searches to get full-page content. Note: WebFetch may return HTTP 403/429 from sites that block server-side fetches (Glassdoor, paywalled coaching, some JS-rendered pages) — if so, report the URL and the status code, do not fall back to Exa, and continue with the URLs you can read. This pattern is itself diagnostic: pages WebFetch can't read are exactly the source class Exa was promoted to cover.

Compile findings into these sections. For every claim cite the URL. Classify each source by tier using the SAME A/B/C rubric as the other agents (judge by the domain). Tag high-impact claims with [Confidence: H/M/L, as of YYYY-MM]. Apply the shared evidence quality standards verbatim.

## Overview (via Web)
What WebSearch surfaces on mission, products, business model.

## Funding & Financials (via Web)
Funding rounds, valuation, investors, revenue signals.

## Leadership (via Web)
Founders, executives, notable hires.

## Recent News & Risks (via Web)
Most recent developments, plus any layoffs/legal/controversy signals.

## Coverage Notes
- Sources WebSearch surfaced that the Exa agents would likely miss (mainstream press, indexed corporate pages, Wikipedia-style overviews)
- URLs you tried to WebFetch that returned 403/429/JS-only — list them; these mark where Exa earns its keep
- Any query that returned thin results
- Publication dates of the freshest sources found
```

### Step 4: Synthesize Results

**Pre-synthesis Read manifest (mandatory):**

Before producing the synthesized dossier, emit a Read manifest as an internal-check step:

```
## Read manifest
**Files Read in full this session (orchestrator):**
- [each candidate-context file actually called via Read — profile.md, professional-identity.md, networking.md, job-pipeline.md, etc.]
- [if refresh: existing dossier file]

**Agent outputs received in full:**
- Agent 1 (Company Overview)
- Agent 2 (Funding & Financials)
- Agent 3 (People & Culture)
- Agent 4 (News & Strategic Context)
- Agent 5 (Competitive Landscape)
- Agent 6 (Exa Neural Search)

**Files/sources referenced by grep/snippet/filename only (NOT Read in full):**
- [list, or "(none)"]
```

**Rule:** Do not make claims about candidate fit, existing connections, or pipeline status from files in the third list. If a "Connection to You" claim or alumni-overlap claim requires content from networking.md / project-index.md / education.md that wasn't fully Read, Read it before writing that section. Partial reads count as "snippet only," not "full Read."

Origin: [[feedback_no_confabulation_in_corpus_synthesis]]. The synthesis step is the highest-risk moment for confabulation because the orchestrator is integrating 5 agent outputs + multiple candidate files; the pull to produce a polished dossier can override grounding.

After the manifest, synthesize the agent findings:
...
### Step 8b: Render PDF (mandatory terminal artifact)

After saving the markdown dossier in Step 8, render a printable PDF as the closing action. Per `memory/feedback_print_prep_pdfs`, Nick reads high-stakes dossiers on paper before live calls.

```bash
PYTHONIOENCODING=utf-8 python3 tools/md_to_pdf_doc.py output/<slug>/<slug>.md
```

Produces `output/<slug>/<slug>.pdf` (weasyprint, Georgia, multi-page).

Then offer print:

> Generated `output/<slug>/<slug>.pdf`. Run `lpr output/<slug>/<slug>.pdf` to send to default printer (recommended for upcoming calls with this company).

Do NOT auto-print without Nick's confirmation.
