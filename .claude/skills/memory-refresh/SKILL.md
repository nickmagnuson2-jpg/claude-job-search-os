...
For demotion candidates: confirm with Nick before archiving anything — a long `last_cited` gap can mean "genuinely stale" or "just hasn't come up," and only Nick can tell the difference. Archive confirmed ones the same way (append to the dated archive file, remove the `MEMORY.md` index line if one exists, don't delete the source).
