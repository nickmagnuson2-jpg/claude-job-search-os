...
user-invocable: true
allowed-tools: Read(*), Read(data/job-pipeline.md), Read(data/networking.md), Read(data/job-todos-daily-log.md), Read(data/outreach-log.md), Glob(output/**), Bash(python3 tools/todo_write.py:*)
---
...
- **Use full names for matching** — match "Acme AI" not just "Acme", match "Jordan Lee" not just "Alex". This prevents false positives on partial name matches.
...
```
  3. Prepare your "why mental health" narrative for Alex coffee chat [High | Due: 2026-02-25]
     > Pipeline: Acme AI — Strategy & Operations [Researching]
     > Contact: Jordan Lee — Product Lead @ Acme AI (peer) — last: 2026-02-18
```
...
5. **Pipeline summary** — at the bottom, show a brief cross-reference of how many to-dos are linked to each active pipeline entry:
   ```
   Pipeline links: Acme AI (4 to-dos) | Stripe (1 to-do)
   ```
...
1. Parse `$ARGUMENTS` to extract:
   ...
   - **Due** (optional, default: none) — date in YYYY-MM-DD format
2. Run the write script (substitute extracted values; use a bare `-` for any omitted priority/due/notes):
   ```bash
   PYTHONIOENCODING=utf-8 python3 tools/todo_write.py add "TASK" "PRIORITY" "DUE" "-"
   ```
   Use a bare `-` (single dash) as the placeholder for any omitted argument — NOT `--` (a `--`-prefixed token reads as a kwarg flag) and NOT `--flag` names. `todo_write.py` is positional-only. You may also simply omit trailing optional args.
3. Parse JSON result:
...
2. Run the write script:
   ```bash
   PYTHONIOENCODING=utf-8 python3 tools/todo_write.py done "FRAGMENT"
   ```
3. Parse JSON result:
...
#### Done
- [x] Read Quiet Capital's Acme investment thesis (Med)
- [x] Update LinkedIn headline (High)

#### Still Active
- [ ] Prepare "why mental health" narrative [High | Due: 2026-02-25]
- [ ] Follow up: Jordan Lee @ Acme AI [Med | Due: 2026-02-25]
- [ ] Read Jordan Lee's care-retention white paper [Med | Due: 2026-02-25]
- [ ] Review Acme AI open roles [Med | Due: 2026-02-25]

#### Research Completed
- Company: Northwind (output/northwind/northwind.md)

#### Outreach Sent
- Sam Carter @ Northwind — email — "Northwind's next chapter"
...
| Company | Role | Stage |
|---------|------|-------|
| Acme AI | Strategy & Operations | Researching |

#### Networking Activity
- Jordan Lee (Acme AI, peer) — last: 2026-02-18, pending to-dos: 2
...
2. Follow up: Jordan Lee @ Acme AI — Check for response [Med | Due: 2026-02-25 | Pending]
   > Pipeline: Acme AI — Strategy & Operations [Researching]
   > Contact: Jordan Lee — Product Lead @ Acme AI (peer) — last: 2026-02-18

3. Read Jordan Lee's operations white paper [Med | Due: 2026-02-25 | Pending]
   > Contact: Jordan Lee — Product Lead @ Acme AI (peer) — last: 2026-02-18

4. Read Quiet Capital's Acme investment thesis [Med | Due: 2026-02-25 | Pending]

5. Review Acme AI open roles for fit [Med | Due: 2026-02-25 | Pending]
   > Pipeline: Acme AI — Strategy & Operations [Researching]

---
Pipeline links: Acme AI (2 to-dos)
Completed (archived): 0 items
```

Note: Cross-referencing uses **full name matching** — "Alex" alone won't match "Jordan Lee", and "Acme" alone won't match "Acme AI". Only to-dos containing the exact full company name or contact name get annotated.
